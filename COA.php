...
<div id="page-wrapper">
	<div class="container-fluid">
		<div class="row">
			<div class="col-lg-12">
				<h1 class="page-header">PT. ELASTOMIX INDONESIA - CERTIFICATE OF ANALYSIS (COA)</h1>
			</div><!-- /.col-lg-12 -->
		</div><!-- /.row -->
					...
					<div class="panel-body">
						<table class="table table-striped table-bordered table-hover" id="tabel-data">
							...
							<tbody>
								<?php
								$i = 1;
								$sql = "SELECT * FROM coa ORDER BY id_coa ASC";
								$ress = mysqli_query($conn, $sql);
								while ($data = mysqli_fetch_array($ress)) {
									echo '<tr>';
									echo '<td class="text-center">' . $i . '</td>';
									echo '<td class="text-center">' . $data['id_coa'] . '</td>';
									echo '<td class="text-center">' . $data['nama_coa'] . '</td>';
									...
									echo '<a href="coavideo.php?video=' . urlencode($data['video']) . '" target="_blank" class="btn btn-warning btn-xs">Lihat Video</a>';
...
<script>
	...
	$('[data-load-code]').on('click', function(e) {
		...
		if (code) {
			$($this.data('remote-target')).load('coaview.php?code=' + code);
			app.code = code;
